<template>
  <div class="star-rating">
    ...
    <div v-if="isIndicatorActive" class="indicator">{{ propRating }}</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    setStars() {
      let fullStarsCounter = Math.floor(this.propRating);
      for (let i = 0; i < this.stars.length; i++) {
        if (fullStarsCounter !== 0) {
          ...
          let surplus = Math.round((this.propRating % 1) * 10) / 10; // Support just one decimal
